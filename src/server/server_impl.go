...
func NewServer(allHostPorts []string, self int, isDebug bool, needFile bool) (Server, error) {
	...
	s := &server{
		...
		closed:       false,
		needFile:     needFile,
		fileName:     "../DB/db_" + allHostPorts[self],
	}
	if needFile {
	...
	//fmt.Println("Server store seq:",s.rid)
...
func (s *server) Get(args *GetArgs, reply *GetReply) error {
	...
	for {
		...
		//fmt.Println("Server store request: "+new_r.Key+" "+new_r.Value+" seq:",s.rid)
...
func (s *server) Put(args *PutArgs, reply *PutReply) error {
	...
	//fmt.Println("Server finish put request: "+r.Key+" "+r.Value)
	return nil
